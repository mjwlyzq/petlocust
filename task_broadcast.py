...
from clients.broadcast import BroadcastClient
...
class PerformanceTest(TaskSet):
    ...
    def get_rank_data(self):
        for i in range(20):
            try:
                # BroadcastClient().live_barrage_audit(self.client, self.userInfo)
                BroadcastClient().custom_barrage(self.client, self.userInfo)
    ...
    @task(0)
    def quick_barrage_data(self):
        for i in range(20):
            try:
                # BroadcastClient().live_barrage_audit(self.client, self.userInfo)
                BroadcastClient().quick_barrage(self.client, self.userInfo)
            except AccountInvalidException as err:
                log.warning(err)
                self.require()
            except ResponseException as err:
    ...
    @task(0)
    def like_barrage_data(self):
        for i in range(20):
            try:
                # BroadcastClient().live_barrage_audit(self.client, self.userInfo)
                BroadcastClient().like_barrage(self.client, self.userInfo)
            except AccountInvalidException as err:
                log.warning(err)
                self.require()
            except ResponseException as err:
    ...
    @task(1)
    def bullet_barrage_data(self):
        for i in range(20):
            try:
                BroadcastClient().live_barrage_audit(self.client, self.userInfo)
                # BroadcastClient().bullet_barrage(self.client, self.userInfo)
                # BroadcastClient().like_submit(self.client, self.userInfo)
                BroadcastClient().test(self.client, self.userInfo)
            except AccountInvalidException as err:
                log.warning(err)
                self.require()
            except ResponseException as err:
...
class WebsiteUser(HttpUser):
    ...
    wait_time = constant_pacing(100000)  # 一个user一秒发一次,间隔1000秒后再发
